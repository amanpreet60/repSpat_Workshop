...
Builds a region-by-region adjacency matrix from
`adata.uns["repspat_mmd_results"]`, connecting regions whose adjusted
p-value is at least `alpha` (i.e. regions that could not be
...
A region-by-region adjacency `DataFrame`. Regions considered similar
(repeated patterns) are connected with their observed MMD as the edge
weight; regions found to be significantly different are left unconnected
(`0`).
